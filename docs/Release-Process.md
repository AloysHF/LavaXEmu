...
This document describes how to publish a new release of LavaXEmu.
...
| `crates/lavaxemu-libretro/lavaxemu_libretro.info` | `display_version` | `"0.1.0"` |

Both values must match. The `.info` file is copied directly into the
...
sed -i 's/^display_version = "0.1.0"/display_version = "0.2.0"/' \
  crates/lavaxemu-libretro/lavaxemu_libretro.info
```

### 2. Commit the version bump

```bash
git add Cargo.toml Cargo.lock crates/lavaxemu-libretro/lavaxemu_libretro.info
...
2. **Builds libretro cores** for the same platforms, renaming the cdylib to
   `lavaxemu_libretro.<ext>` and bundling `lavaxemu_libretro.info`
...
1. Go to [Releases](https://github.com/AloysHF/LavaXEmu/releases)
...
4. Verify all expected artifacts are attached:
   - `lavaxemu-linux-x86_64.tar.gz`
   - `lavaxemu-macos-x86_64.tar.gz`
   - `lavaxemu-macos-aarch64.tar.gz`
   - `lavaxemu-windows-x86_64.zip`
...
(`dist/info/lavaxemu_libretro.info`), not from this repo. If the `.info` file
...
2. Copy `crates/lavaxemu-libretro/lavaxemu_libretro.info` from this repo
   to `dist/info/lavaxemu_libretro.info` in the fork
3. Submit a PR to `libretro/libretro-super` — reference the LavaXEmu release
...
- Check the [Actions](https://github.com/AloysHF/LavaXEmu/actions) tab
  for the failed run
...
[Releases](https://github.com/AloysHF/LavaXEmu/releases) page before
...
- Ensure both files have the same version string:
  `Cargo.toml` and `lavaxemu_libretro.info`
