<#
.SYNOPSIS
    Capture screenshots for LAV files.

.DESCRIPTION
    Runs the standalone emulator for every LAV file under the selected game
...
.PARAMETER Frames
    Number of frames to run before capturing. Default: 300.

.PARAMETER Binary
    Path to the lavaxemu executable. Default: the release build output.

.PARAMETER GameDirectory
    Directory searched recursively for LAV files.
...
if (-not $GameDirectory) {
    $GameDirectory = Join-Path $repoRoot "tmp\lavax_game"
}
...
if (-not $Binary) {
    $Binary = Join-Path $repoRoot "target\release\lavaxemu.exe"
    if ($env:OS -ne "Windows_NT") {
        $Binary = Join-Path $repoRoot "target/release/lavaxemu"
    }

    Write-Host "Building the latest release binary..." -ForegroundColor Yellow
    try {
        Push-Location $repoRoot
        cargo build --release -p lavaxemu --bin lavaxemu
...
if (-not (Test-Path -LiteralPath $Binary -PathType Leaf)) {
    Write-Error "lavaxemu binary not found: $Binary"
    exit 1
}

$games = @(Get-ChildItem -LiteralPath $GameDirectory -Recurse -File |
    Where-Object { $_.Extension -ieq ".lav" } |
    Sort-Object FullName)

if ($games.Count -eq 0) {
    Write-Warning "No LAV files found under $GameDirectory"
    exit 0
}
...
foreach ($game in $games) {
    ...
    try {
        $output = & $Binary $game.FullName --screenshot $outPath `
            --frames $captureFrames --read-only 2>&1
